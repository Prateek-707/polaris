...
if not IS_POLARIS:
    ...
    from workloads.ttnn.vgg_unet.wh.vgg_unet_test_infra_polaris_wh import create_test_infra  # type: ignore[no-redef]
...
def test_vgg_unet_perf_device(batch_size: int = 1, expected_kernel_samples_per_sec: int = 198):
    ...
    command = (
        f'pytest models/demos/vision/segmentation/vgg_unet/wormhole/tests/'
        f'test_vgg_unet_device_perf.py::test_vgg_unet_device_ops[{batch_size}-device_params0]'
    )
...
_STANDALONE_RUN_SPECS: list[tuple[str, object, str]] = [
    ('device-ops', run_vgg_unet_device_ops, 'vgg-unet-wh-device-ops'),
]
...
def standalone(test_name: str | None = None) -> None:
    """Run standalone WH device-perf VGG UNet tests, or a single test by short name."""
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run VGG UNet (Wormhole) device-perf standalone tests.'
    )
    parser.add_argument(
        'test',
        nargs='?',
