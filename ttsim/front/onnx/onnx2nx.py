...
_DIM_NAME_ALIASES: dict[str, str] = {
    ...
    "sequence_length": "seq_len", "seq_len": "seq_len", "S": "seq_len",
...
def parse_value_info_list(vlist):
    ...
    for vi in vlist:
        if vi.name in tbl:
            ...
            logger.warning(
                "Duplicate value_info '{}' encountered; keeping first and skipping duplicate.",
                vi.name,
            )
            continue
...
def resolve_tensor(_tname, _info, dim_overrides: Optional[dict] = None):
    ...
    if dims is None:
        logger.warning(
        "ONNX tensor '{}' has no shape information (dims=None); "
